...
function app(app) {
	...
	app.post('/upload', function (req, res) {
	  	/*for (var i in req.files) {
	    	if (req.files[i].size === 0) {
	      		...
	      		var target_path = './public/imgs/' + req.files[i].name;
	      		// 使用同步方式重命名一个文件
	  	...
	  	var jsontest = {error: error, imgURL: imgURL};
	  	res.jsonp(jsontest);
	});
	//post
	...
	app.post('/delPost', function (req, res) {
		var id = req.body.id,
				...
				page = req.body.p;
		Posts.removePost(id, userId, function (err, result) {
			if (err) {
				console.log(err);
			}
			if (result) {
	...
	app.get('/ajax', function (req, res) {
		Posts.findTen(function (err, posts) {
			if (err) {
				console.log(err);
			}
